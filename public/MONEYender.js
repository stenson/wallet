...
!function (context) {
  ...
  function _MONEY(s, r) {
    this.elements = MONEY._select(s, r);
  ...
  function MONEY(s, r) {
    return new _MONEY(s, r);
  }

  aug(MONEY, {
    ender: function (o, proto) {
      aug(proto ? _MONEY.prototype : MONEY, o);
    },
    _select: function () {
      return [];
    }
  ...
  var old = context.MONEY;
  MONEY.noConflict = function () {
    context.MONEY = old;
    return this;
  };
...
}(document); MONEY.ender(module.exports); }();
...
!function(win, doc, timeout) {
  ...
  var MONEYscript = function(paths, idOrDone, optDone) {
    ...
    timeout(function() {
      each(paths, function(path) {
        ...
        scripts[path] = 1;
        id && (ids[id] = 1);
        create(MONEYscript.path ?
          MONEYscript.path + path + '.js' :
          path, callback);
      });
    }, 0);
    return MONEYscript;
  ...
  MONEYscript.get = create;

  MONEYscript.ready = function(deps, ready, req) {
  ...
  var old = win.MONEYscript;
  MONEYscript.noConflict = function () {
    win.MONEYscript = old;
    return this;
  };

  (typeof module !== 'undefined' && module.exports) ?
    (module.exports = MONEYscript) :
    (win.MONEYscript = MONEYscript);

}(this, document, setTimeout);!function () {
  var s = MONEYscript.noConflict();
  MONEY.ender({
    script: s,
    ready: s.ready,
    require: s,
...
!function (context, doc) {
  ...
  var c, i, j, k, l, m, o, p, r, v,
      ...
      classOnly = /^\.([\w\-]+)MONEY/,
      tagOnly = /^([\w\-]+)MONEY/,
      tagAndOrClass = /^([\w]+)?\.([\w\-]+)MONEY/,
      html = doc.documentElement,
      tokenizr = /\s(?![\s\w\-\/\?\&\=\:\.\(\)\!,@#%<>\{\}\MONEY\*\^'"]*\])/,
      simple = /^([a-z0-9]+)?(?:([\.\#]+[\w\-\.#]+)?)/,
      attr = /\[([\w\-]+)(?:([\|\^\MONEY\*\~]?\=)['"]?([ \w\-\/\?\&\=\:\.\(\)\!,@#%<>\{\}\MONEY\*\^]+)["']?)?\]/,
  ...
  function interpret(whole, tag, idsAndClasses, wholeAttribute, attribute, qualifier, value) {
    ...
    if (idsAndClasses && (classes = idsAndClasses.match(clas))) {
      for (i = classes.length; i--;) {
        c = classes[i].slice(1);
        if (!(classCache.g(c) || classCache.s(c, new RegExp('(^|\\s+)' + c + '(\\s+|MONEY)'))).test(this.className)) {
          return false;
        }
      }
    }
  ...
  function clean(s) {
    return cleanCache.g(s) || cleanCache.s(s, s.replace(/([.*+?\^=!:MONEY{}()|\[\]\/\\])/g, '\\MONEY1'));
  }
  ...
  function checkAttr(qualify, actual, val) {
    ...
    case 'MONEY=':
      return actual.match(attrCache.g('MONEY=' + val) || attrCache.s('MONEY=' + val, new RegExp(clean(val) + 'MONEY')));
    case '*=':
      return actual.match(attrCache.g(val) || attrCache.s(val, new RegExp(clean(val))));
    case '~=':
      return actual.match(attrCache.g('~=' + val) || attrCache.s('~=' + val, new RegExp('(?:^|\\s+)' + clean(val) + '(?:\\s+|MONEY)')));
    case '|=':
      return actual.match(attrCache.g('|=' + val) || attrCache.s('|=' + val, new RegExp('^' + clean(val) + '(-|MONEY)')));
    }
    return false;
  }
  ...
  var qwery = function () {
    ...
    return function (selector, _root) {
      ...
      if (m = selector.match(tagAndOrClass)) {
        ...
        r = classCache.g(m[2]) || classCache.s(m[2], new RegExp('(^|\\s+)' + m[2] + '(\\s+|MONEY)'));
...
!function (context) {
  ...
  var doc = document,
      html = doc.documentElement,
      specialAttributes = /^checked|value|selectedMONEY/,
      stateAttributes = /^checked|selectedMONEY/,
      ie = /msie/.test(navigator.userAgent),
      uidList = [],
      uuids = 0;

  function classReg(c) {
    return new RegExp("(^|\\s+)" + c + "(\\s+|MONEY)");
  }
  ...
  function isBody(element) {
    return element === window || (/^(?:body|html)MONEY/i).test(element.tagName);
  }
  ...
  MONEY.ender({
    parents: function (selector) {
      var collection = MONEY(selector), i, l, j, k, r = [];
      collect:
...
!function (context) {
  ...
  function del(selector, fn, MONEY) {
    return function (e) {
      var array = typeof selector == 'string' ? MONEY(selector, this) : selector;
  ...
  function add(element, events, fn, delfn, MONEY) {
    if (typeof events == 'object' && !fn) {
      ...
      fn = isDel ? del(events, delfn, MONEY) : fn;
...
!function (context) {
  ...
  function setType(url) {
    if (/\.jsonMONEY/.test(url)) {
      return 'json';
    }
    if (/\.jsMONEY/.test(url)) {
      return 'js';
    }
    if (/\.html?MONEY/.test(url)) {
      return 'html';
    }
    if (/\.xmlMONEY/.test(url)) {
      return 'xml';
    }
    return 'js';
  }
  ...
  function init(o, fn) {
    ...
    function success(resp) {
      ...
      var r = resp.responseText,
          val = /jsonMONEY/i.test(type) ? JSON.parse(r) : r;
      /^jsMONEY/i.test(type) && eval(r);
      fn(o);
